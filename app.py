# app.py - ENTERPRISE GRADE KNOWLEDGE GRAPH PLATFORM (FIXED)
...
def render_communications_panel(results: List[Dict]):
    ...
    for i, r in enumerate(results):
        sender = r['metadata'].get('from', 'Unknown').split('@')[0]
        st.markdown(f"""
        <div class="email-row">
            ...
            <div style="font-weight: 500; margin-bottom: 0.3rem;">{r['metadata'].get('subject', '')}</div>
            <div style="color: var(--text-light); font-size: 0.85rem; margin-bottom: 0.5rem;">{r['content'][:150]}...</div>
            <div style="display: flex; gap: 0.5rem;">
                <span class="risk-indicator risk-low">confidence {r['score']:.0%}</span>
                <span class="risk-indicator risk-medium">relevance high</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
